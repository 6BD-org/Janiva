...
import com.oracle.truffle.jx.nodes.interop.NodeObjectDescriptor;
 ...
 *
 * <p>We use the primitive type only when the same primitive type is uses for all writes. If the
 * local variable is type-polymorphic, then the value is always stored as an Object, i.e., primitive
 * values are boxed. Even a mixture of {@code long} and {@code boolean} writes leads to both being
 * stored boxed.
 */
@NodeField(name = "slot", type = int.class)
public abstract class JXReadLocalVariableNode extends JXExpressionNode {
  ...
  public Object getNodeObject() {
    return NodeObjectDescriptor.readVariable(
