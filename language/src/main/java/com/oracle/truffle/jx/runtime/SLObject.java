 ...
 *
 * <p>This class defines operations that can be performed on SL Objects. While we could define all
 * these operations as individual AST nodes, we opted to define those operations by using {@link
 * com.oracle.truffle.api.library.Library a Truffle library}, or more concretely the {@link
 * InteropLibrary}. This has several advantages, but the primary one is that it allows SL objects to
 * be used in the interoperability message protocol, i.e. It allows other languages and tools to
 * operate on SL objects without necessarily knowing they are SL objects.
 *
 * <p>SL Objects are essentially instances of {@link DynamicObject} (objects whose members can be
...
public final class SLObject extends DynamicObject implements TruffleObject {
  ...
  void removeMember(
    ...
    TruffleString memberTS = fromJavaStringNode.execute(member, JSONXLang.STRING_ENCODING);
  ...
  boolean existsMember(
      ...
      @Cached @Shared("fromJavaStringNode") TruffleString.FromJavaStringNode fromJavaStringNode,
      @CachedLibrary("this") DynamicObjectLibrary objectLibrary) {
    return objectLibrary.containsKey(
        this, fromJavaStringNode.execute(member, JSONXLang.STRING_ENCODING));
  }
  ...
  Object readMember(
      ...
      throws UnknownIdentifierException {
    Object result =
        objectLibrary.getOrDefault(
            this, fromJavaStringNode.execute(name, JSONXLang.STRING_ENCODING), null);
    if (result == null) {
  ...
  void writeMember(
      ...
      @Cached @Shared("fromJavaStringNode") TruffleString.FromJavaStringNode fromJavaStringNode,
      @CachedLibrary("this") DynamicObjectLibrary objectLibrary) {
    objectLibrary.put(this, fromJavaStringNode.execute(name, JSONXLang.STRING_ENCODING), value);
  }
}
