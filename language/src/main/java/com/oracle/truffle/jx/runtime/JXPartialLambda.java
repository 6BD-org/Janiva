...
import com.oracle.truffle.jx.constants.Constants;import com.oracle.truffle.jx.statics.lambda.LambdaTemplate;
...
public class JXPartialLambda extends DynamicObject implements TruffleObject {
  ...
  @ExportMessage(library = InteropLibrary.class)
  public boolean isBoolean(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library) {
    return library.isBoolean(execute(EMPTY_ARGS));
  }

  @ExportMessage
  public boolean asBoolean(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library)
      throws UnsupportedMessageException {
    return library.asBoolean(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public boolean isString(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library) {
    return library.isString(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public String asString(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library)
      throws UnsupportedMessageException {
    return library.asString(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public boolean isNumber(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library) {
    return library.isNumber(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public boolean fitsInByte(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library) {
    return library.fitsInByte(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public byte asByte(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library)
      throws UnsupportedMessageException {
    return library.asByte(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public boolean fitsInShort(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library) {
    return library.fitsInShort(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public short asShort(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library)
      throws UnsupportedMessageException {
    return library.asShort(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public boolean fitsInInt(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library) {
    return library.fitsInInt(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public int asInt(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library)
      throws UnsupportedMessageException {
    return library.asInt(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public boolean fitsInLong(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library) {
    return library.fitsInLong(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public long asLong(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library)
      throws UnsupportedMessageException {
    return library.asLong(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public boolean fitsInDouble(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library) {
    return library.fitsInDouble(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public double asDouble(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library)
      throws UnsupportedMessageException {
    return library.asDouble(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public boolean fitsInFloat(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library) {
    return library.fitsInFloat(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public float asFloat(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library)
      throws UnsupportedMessageException {
    return library.asFloat(execute(EMPTY_ARGS));
  }
  @ExportMessage(library = InteropLibrary.class)
  public boolean fitsInBigInteger(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library) {
    return library.fitsInBigInteger(execute(EMPTY_ARGS));
  }
  @ExportMessage(library = InteropLibrary.class)
  public BigInteger asBigInteger(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library)throws UnsupportedMessageException {
    return library.asBigInteger(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public boolean hasMembers(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library) {
    return library.hasMembers(execute(EMPTY_ARGS));
  }

  @ExportMessage(library = InteropLibrary.class)
  public Object getMembers(
      boolean includeInternal, @CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library)
      throws UnsupportedMessageException {
  ...
  public boolean isMemberReadable(
      String member, @CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library) {
  ...
  public void writeMember(
      String member,
      Object value,
      @CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library)
  ...
  @ExportMessage(library = InteropLibrary.class)
  public long getArraySize(@CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library)
      throws UnsupportedMessageException {
  ...
  public Object readArrayElement(
      long index, @CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library)
      throws InvalidArrayIndexException, UnsupportedMessageException {
    return library.readArrayElement(execute(EMPTY_ARGS), index);
  }
  @ExportMessage(library = InteropLibrary.class)
  public boolean isArrayElementReadable(long index, @CachedLibrary(limit = DELEGATE_LIB_CACHE) InteropLibrary library) {
