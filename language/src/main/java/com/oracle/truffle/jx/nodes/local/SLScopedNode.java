 ...
 * SOFTWARE.
 */
package com.oracle.truffle.jx.nodes.local;
...
public abstract class SLScopedNode extends Node {
  ...
  final Object getScope(
      Frame frame,
      boolean nodeEnter,
      @Shared("node") @Cached(value = "this", adopt = false) SLScopedNode cachedNode,
  ...
  /**
   * Set the index to the the {@link SLBlockNode#getDeclaredLocalVariables() block's variables} that
   * determine variables belonging into this scope (excluding parent scopes) on node enter.
   */
  ...
  static final class ArgumentsObject implements TruffleObject {
    ...
    static final class ExistsMember {
      ...
      /**
       * If the member is cached, provide the cached result. Call {@link #doGeneric(ArgumentsObject,
       * String)} otherwise.
       */
      @Specialization(
          limit = "LIMIT",
    ...
    static final class ReadMember {
      ...
      /**
       * If the member is cached, use the cached index and read the value at that index. Call {@link
       * #doGeneric(ArgumentsObject, String)} otherwise.
       */
      @Specialization(
          limit = "LIMIT",
          guards = {"cachedMember.equals(member)"})
    ...
    static final class WriteMember {
      ...
      /** Write the argument value at the provided index into {@link Frame#getArguments()} array. */
  ...
  static final class VariablesObject implements TruffleObject {
    ...
    /**
     * For performance reasons, fix the library implementation for the current node and enter flag.
     */
    @ExportMessage
    boolean accepts(
        @Cached(value = "this.node", adopt = false) SLScopedNode cachedNode,
    ...
    Object toDisplayString(
        ...
        @Shared("parentBlock")
            @Cached(value = "this.block.findBlock()", adopt = false, allowUncached = true)
            Node parentBlock) {
    ...
    static final class ExistsMember {
      ...
      /**
       * If the member is cached, provide the cached result. Call {@link #doGeneric(VariablesObject,
       * String)} otherwise.
       */
      @Specialization(
          limit = "LIMIT",
      ...
      /**
       * Test if a variable with that name exists. It exists if we have a corresponding write node.
       */
      @Specialization(replaces = "doCached")
      @TruffleBoundary
    ...
    static final class ReadMember {
      ...
      /**
       * If the member is cached, use the cached frame slot and read the value from it. Call {@link
    ...
    /**
     * Get the variables. Cache the array of write nodes that declare variables in the scope(s) and
     * the indexes which determine visible variables.
     */
    @ExportMessage
    @SuppressWarnings("static-method")
    Object getMembers(
        ...
        @Cached(
                value = "this.block.getDeclaredLocalVariables()",
                adopt = false,
                dimensions = 1,
                allowUncached = true)
  ...
  static final class KeysArray implements TruffleObject {
     ...
     * @param variableIndex index to the variables array, determining variables in the inner-most
     *     scope (from zero index up to the <code>variableIndex</code>, exclusive).
