...
public class SLNodeFactory {
  ...
  private SLExpressionNode rootNode;
  ...
  public void finishFunction(SLStatementNode bodyNode) {
    if (bodyNode == null) {
      ...
      final SLStatementNode methodBlock =
          finishBlock(
  ...
  public void registerRootNode(SLExpressionNode node) {
    frameDescriptorBuilder = FrameDescriptor.newBuilder();
    this.rootNode = node;
  }

  public RootNode getRootNode() {
    return new SLRootNode(
            language, frameDescriptorBuilder.build(), rootNode, SLStrings.fromJavaString("#root")
    );
  }


  public SLExpressionNode createDecimal(Token whole, Token dec) {
    if (dec == null) {
      return new SLNumberLiteralNode(new BigDecimal(whole.getText()));
    }
    return new SLNumberLiteralNode(new BigDecimal(whole.getText() + "." + dec.getText()));
  }
  ...
  public SLStatementNode finishBlock(
      List<SLStatementNode> bodyNodes, int skipCount, int startPos, int length) {
    lexicalScope = lexicalScope.outer;
    ...
    SLBlockNode blockNode =
        new SLBlockNode(flattenedNodes.toArray(new SLStatementNode[flattenedNodes.size()]));
  ...
  private void flattenBlocks(
      Iterable<? extends SLStatementNode> bodyNodes, List<SLStatementNode> flattenedNodes) {
   ...
   *     bodyNode is null.
   */
  public SLStatementNode createWhile(
      Token whileToken, SLExpressionNode conditionNode, SLStatementNode bodyNode) {
   ...
   *     null.
   */
  public SLStatementNode createIf(
      Token ifToken,
   ...
   */
  public SLExpressionNode createBinary(
      Token opToken, SLExpressionNode leftNode, SLExpressionNode rightNode) {
   ...
   */
  public SLExpressionNode createCall(
      SLExpressionNode functionNode, List<SLExpressionNode> parameterNodes, Token finalToken) {
   ...
   * @return An SLExpressionNode for the given parameters. null if nameNode or valueNode is null.
   */
  public SLExpressionNode createAssignment(
      SLExpressionNode nameNode, SLExpressionNode valueNode, Integer argumentIndex) {
    ...
    final SLExpressionNode result =
        SLWriteLocalVariableNodeGen.create(valueNode, frameSlot, nameNode, newVariable);
  ...
  /**
   * Returns a {@link SLReadLocalVariableNode} if this read is a local variable or a {@link
   * SLFunctionLiteralNode} if this read is global. In SL, the only global names are functions.
   *
  ...
  public SLExpressionNode createStringLiteral(Token literalToken, boolean removeQuotes) {
    final SLStringLiteralNode result =
        new SLStringLiteralNode(asTruffleString(literalToken, removeQuotes));
  ...
  public SLExpressionNode createParenExpression(
      SLExpressionNode expressionNode, int start, int length) {
   ...
   */
  public SLExpressionNode createReadProperty(
      SLExpressionNode receiverNode, SLExpressionNode nameNode) {
  ...
  public SLExpressionNode createWriteProperty(
      SLExpressionNode receiverNode, SLExpressionNode nameNode, SLExpressionNode valueNode) {
    if (receiverNode == null || nameNode == null || valueNode == null) {
      return null;
    }

    final SLExpressionNode result =
        SLWritePropertyNodeGen.create(receiverNode, nameNode, valueNode);
