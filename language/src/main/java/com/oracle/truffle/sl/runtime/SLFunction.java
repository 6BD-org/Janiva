 ...
 * RootCallTarget call target}. This class encapsulates a call target, and adds version support:
 * functions in SL can be redefined, i.e. changed at run time. When a function is redefined, the
 * call target managed by this function object is changed (and {@link #callTarget} is therefore not
 * a final field).
 *
 * <p>Function redefinition is expected to be rare, therefore optimized call nodes want to speculate
...
public final class SLFunction implements TruffleObject {
  ...
  /**
   * Manages the assumption that the {@link #callTarget} is stable. We use the utility class {@link
   * CyclicAssumption}, which automatically creates a new {@link Assumption} when the old one gets
   * invalidated.
   */
   ...
   *
   * <p>Since invocations are potentially expensive (result in an indirect call, which is expensive
   ...
   * This ensures that the runtime is notified when this node turns polymorphic. This, in turn, may,
   * under certain conditions, cause the runtime to attempt to make node monomorphic again by
   * duplicating the entire AST containing that node and specialising it for a particular call site.
   */
  @ReportPolymorphism
  @ExportMessage
  abstract static class Execute {
     ...
     *
     * <p>Since SL is a quite simple language, the benefit of the inline cache seems small: after
     ...
     *
     * <p>{@code limit = "INLINE_CACHE_SIZE"} Specifies the limit number of inline cache
     * specialization instantiations.
     *
     * <p>{@code guards = "function.getCallTarget() == cachedTarget"} The inline cache check. Note
     * that cachedTarget is a final field so that the compiler can optimize the check.
     *
     * <p>{@code assumptions = "callTargetStable"} Support for function redefinition: When a
     * function is redefined, the call target maintained by the SLFunction object is changed. To
     * avoid a check for that, we use an Assumption that is invalidated by the SLFunction when the
     * change is performed. Since checking an assumption is a no-op in compiled code, the assumption
